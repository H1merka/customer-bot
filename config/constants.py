...
DEFAULT_CUSTOM_TEXTS = {
    ...
    "cert_prepayment_info": "Для покупки сертификата на сумму {amount} руб., пожалуйста, переведите полную стоимость ({amount} руб.) по номеру телефона <code>89630313100</code> (Т-Банк, получатель Юлия С.).\n\n⚠️ <b>Внимание:</b> получатель сертификата должен быть здоров и совершеннолетен (либо иметь согласие родителей на процедуру).\n\nПосле оплаты администратор проверит перевод.",
...
def clear_booking_session(user_data: dict) -> None:
    ...
    booking_keys = [
        ...
        "cert_target_service",
        "cert_amount",
        "cert_recipient_tg",  # Новый ключ для ссылки на получателя
    ]
